...
function Home() {
  ...
  const { fetchNextPage, isFetching, hasNextPage } = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages) => {
      if (!lastPage || typeof lastPage.prevPage !== 'number') return undefined
      const loadedPosts = allPages.reduce((acc, page) => acc + (page?.posts?.length || 0), 0)
      if (loadedPosts >= lastPage?.total) return undefined
